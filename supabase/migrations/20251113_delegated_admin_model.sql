-- Migration: Implement Delegated Admin Management (Model B)
-- SUPERADMIN → PRIMARY ADMIN → SECONDARY ADMINS → USERS

-- Step 1: Update user_profiles to support new role hierarchy
DO $$
BEGIN
  -- Check if role column needs to be updated to support new roles
  -- Existing roles: 'admin', 'user'
  -- New roles: 'primary_admin', 'secondary_admin', 'user'

  -- Update existing 'admin' users to 'primary_admin' (one per org)
...
-- Step 2: Add column to track primary admin per organization
ALTER TABLE organizations
ADD COLUMN IF NOT EXISTS primary_admin_id UUID REFERENCES user_profiles(id);

-- Step 3: Create function to count secondary admins per organization
...
-- Step 4: Create function to check if user is primary admin of their org
...
-- Step 5: Create function to check if user can create secondary admin
...
-- Step 6: Update RLS policies for user_profiles to respect new hierarchy
...
-- Step 7: Migrate existing admins to primary_admin
-- This updates existing 'admin' users to 'primary_admin'
UPDATE user_profiles
SET role = 'primary_admin'
WHERE role = 'admin';

-- Step 8: Link primary admins to their organizations
...
-- Step 9: Create helper function for Super Admin panel
