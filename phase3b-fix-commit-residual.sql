-- =====================================================
-- FIX: commit_user_period to calculate residual values
-- The risks table doesn't store residual values, we need to calculate them
-- =====================================================
...
BEGIN
    ...
    -- Copy risks to risk_history WITH controls and CALCULATED residual values
    -- We need to calculate residual values based on controls effectiveness
    ...
    SELECT
        ...
        -- Calculate residual likelihood: inherent - (sum of likelihood control reductions)
        GREATEST(1, r.likelihood_inherent - COALESCE(
            (SELECT SUM(
                CASE
                    WHEN c.target = 'Likelihood'
                    THEN (c.design_rating + c.implementation_rating + c.monitoring_rating + c.effectiveness_evaluation) / 4.0
                    ELSE 0
                END
            ) FROM controls c WHERE c.risk_id = r.id),
            0
        ))::numeric,
        -- Calculate residual impact: inherent - (sum of impact control reductions)
        GREATEST(1, r.impact_inherent - COALESCE(
            (SELECT SUM(
                CASE
                    WHEN c.target = 'Impact'
                    THEN (c.design_rating + c.implementation_rating + c.monitoring_rating + c.effectiveness_evaluation) / 4.0
                    ELSE 0
                END
            ) FROM controls c WHERE c.risk_id = r.id),
...
COMMENT ON FUNCTION commit_user_period IS 'Commits all user risks for a period to risk_history with controls and calculated residual values';
