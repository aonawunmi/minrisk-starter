...
// =====================================================
function parsePortfolioHoldings(worksheet) {
    ...
    for (const row of data) {
        ...
        const assetType = cleanString(row.Asset_Type || row['Asset Type'] || row.asset_type);
...
// PARSE PRICE HISTORY
// =====================================================
function parsePriceHistory(worksheet) {
    const data = XLSX.utils.sheet_to_json(worksheet);
    const priceHistory = [];
    for (const row of data) {
...
// PARSE CONFIGURATION
// =====================================================
function parseConfiguration(worksheet) {
    const data = XLSX.utils.sheet_to_json(worksheet);
    const configMap = {};
    for (const row of data) {
    ...
    // Parse with defaults
    const dataFrequency = cleanString(configMap.Data_Frequency || configMap.data_frequency || 'Daily');
    const confidenceLevelStr = cleanString(configMap.Confidence_Level || configMap.confidence_level || '95%');
    const confidenceLevel = parseFloat(confidenceLevelStr.replace('%', ''));
...
// MAIN PARSE FUNCTION
// =====================================================
export function parseVarExcelFile(file) {
