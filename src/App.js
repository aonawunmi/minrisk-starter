...
import { Plus, RefreshCw, Settings, Table, Pencil, Trash2, ChevronsUpDown, FileUp, AlertTriangle, ArrowUpDown, Sparkles, Calendar, Archive } from "lucide-react";
...
function HeatmapTab({ processedData, allRows, uniquePeriods, heatMapView, setHeatMapView, priorityRisks, config, onEditRisk, canEdit }) {
    ...
    return (_jsx(Card, { className: "rounded-2xl shadow-sm", children: _jsxs(CardContent, { className: "p-4", children: [_jsxs("div", { className: "mb-3 space-y-3", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { className: "text-sm text-gray-500", children: [dataSource === 'active'
                                        ...
                                        return (_jsxs(Popover, { children: [_jsx(PopoverTrigger, { asChild: true, children: _jsx("div", { className: "h-20 border border-gray-200 flex items-center justify-center p-1 relative cursor-pointer", style: { backgroundColor: `${bgColor}E6` }, children: _jsxs("div", { className: "flex gap-2 text-lg font-bold", children: [heatMapView.inherent && cell.inherent.length > 0 && _jsx("span", { className: "text-blue-700", children: cell.inherent.length }), heatMapView.inherent && heatMapView.residual && cell.inherent.length > 0 && cell.residual.length > 0 && _jsx("span", { className: "text-gray-400", children: "/" }), heatMapView.residual && cell.residual.length > 0 && _jsx("span", { className: "text-rose-700", children: cell.residual.length })] }) }) }), allRisksInCell.length > 0 && (_jsxs(PopoverContent, { children: [_jsxs("div", { className: "font-bold text-sm mb-2", children: ["Risks in cell (L:", likelihood, ", I:", impact, ")"] }), _jsxs("div", { className: "max-h-60 overflow-y-auto", children: [heatMapView.inherent && cell.inherent.length > 0 && (_jsxs("div", { children: [_jsx("h4", { className: "font-semibold text-blue-700 mt-2", children: "Inherent Position" }), cell.inherent.map(risk => (_jsxs("button", { className: "w-full text-left border-b p-2 text-xs hover:bg-gray-100", onClick: () => onEditRisk(risk), children: [_jsxs("p", { className: "font-bold", children: [risk.risk_code, ": ", risk.risk_title] }), _jsxs("p", { className: "text-gray-600 text-xs", children: ["(Residual L: ", risk.likelihood_residual.toFixed(1), ", I: ", risk.impact_residual.toFixed(1), ")"] })] }, risk.risk_code)))] })), heatMapView.residual && cell.residual.length > 0 && (_jsxs("div", { children: [_jsx("h4", { className: "font-semibold text-rose-700 mt-2", children: "Residual Position" }), cell.residual.map(risk => (_jsx("button", { className: "w-full text-left border-b p-2 text-xs hover:bg-gray-100", onClick: () => onEditRisk(risk), children: _jsxs("p", { className: "font-bold", children: [risk.risk_code, ": ", risk.risk_title] }) }, risk.risk_code)))] }))] })] }))] }, `${likelihood}-${impact}`));
                                    }))) }), _jsx("div", { className: "flex justify-between pl-8 pr-8", children: Array.from({ length: config.matrixSize }, (_, i) => i + 1).map(lik => (_jsx("div", { className: "w-20 text-center text-xs font-semibold", children: config.likelihoodLabels[lik - 1] }, lik))) })] })] })] }) }));
}
