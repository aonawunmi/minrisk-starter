...
export default function AuthGate({ children }) {
    ...
    const checkIfPasswordSet = (session) => {
    ...
    if (error) {
        return (_jsx("div", { className: "min-h-screen w-full bg-gray-50 flex items-center justify-center", children: _jsx("div", { className: "bg-white rounded-lg shadow-lg p-8 max-w-md", children: _jsxs("div", { className: "text-center", children: [_jsx("div", { className: "inline-flex items-center justify-center w-16 h-16 bg-red-100 rounded-full mb-4", children: _jsx("span", { className: "text-red-600 text-2xl", children: "\u26A0\uFE0F" }) }), _jsx("h2", { className: "text-xl font-bold text-gray-900 mb-2", children: "Authentication Error" }), _jsx("p", { className: "text-gray-600 mb-4", children: error }), _jsx("button", { onClick: () => window.location.reload(), className: "bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700", children: "Reload Page" })] }) }) }));
    }
    if (loading) {
    ...
    if (needsPasswordSetup) {
        return (_jsx(SetPasswordPage, { onPasswordSet: handlePasswordSet, userEmail: session.user?.email || null, organizationName: session.user?.user_metadata?.organization_name || null }));
    }
