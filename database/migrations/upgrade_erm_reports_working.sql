...
-- WORKING VERSION: Proper type handling for TEXT user_id
...
-- Note: user_id in risks table is TEXT, auth.uid() returns UUID, so we cast auth.uid() to TEXT
...
USING (
    organization_id IN (
        SELECT DISTINCT organization_id FROM risks WHERE user_id = auth.uid()::text
    )
);
