...
-- FIXED VERSION: Simplified RLS policies for MinRisk schema
...
-- Organization Settings: Simple policy based on organization_id matching user's risks
ALTER TABLE organization_settings ENABLE ROW LEVEL SECURITY;

CREATE POLICY org_settings_select ON organization_settings FOR SELECT
USING (
    organization_id IN (
        SELECT DISTINCT organization_id FROM risks WHERE user_id = auth.uid()
    )
);
