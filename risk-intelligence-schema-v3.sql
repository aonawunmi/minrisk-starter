...
-- Remove duplicate risks, keeping only the most recent one
DO $$
DECLARE
  duplicate_record RECORD;
BEGIN
  -- Find and delete duplicates, keeping the most recent (highest id)
  FOR duplicate_record IN
    SELECT organization_id, risk_code, MAX(id) as keep_id
    FROM risks
  ...
  LOOP
    ...
    RAISE NOTICE 'Removed duplicates for risk_code: % in org: %',
      duplicate_record.risk_code, duplicate_record.organization_id;
  END LOOP;
END $$;
...
-- It keeps the most recent entry (highest ID) and deletes older duplicates
